...
class CNN(object):
    def __init__(self):
        ...
        if AlexNet_Or_LeNet:
            # 第1层卷积，卷积核大小为11*11，64个，28*28为待训练图片的大小
            model.add(
                layers.Conv2D(64, (11, 11), activation='relu', padding='same', input_shape=(28, 28, 1)))  # 11*11*64
            model.add(layers.MaxPooling2D((2, 2), strides=[2, 2]))
            # model.add(layers.BatchNormalization(axis= 1))

            # 第2层卷积，卷积核大小为5*5，192个
            model.add(layers.Conv2D(192, (5, 5), activation='relu', padding='same'))  # 5*5*192
            model.add(layers.MaxPooling2D((2, 2), strides=[2, 2]))
            # model.add(layers.BatchNormalization(axis=1))
            # 第3、4、5层卷积，卷积核大小为3*3
            model.add(layers.Conv2D(384, (3, 3), activation='relu', padding='same'))  # 3*3*384
            # model.add(layers.BatchNormalization(axis=1))
            model.add(layers.Conv2D(384, (3, 3), activation='relu', padding='same'))  # 3*3*384
            # model.add(layers.BatchNormalization(axis=1))
            model.add(layers.Conv2D(256, (3, 3), activation='relu', padding='same'))  # 3*3*256
            model.add(layers.MaxPooling2D((2, 2), strides=[2, 2]))
            # model.add(layers.BatchNormalization(axis=1))
...
class Train:
    ...
    def train(self):
        ...
        # Save model as well
        self.cnn.model.save_weights("newWeight.hdf5", overwrite=True)
        print("保存.hdf5文件成功")
